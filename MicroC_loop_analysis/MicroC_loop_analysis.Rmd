...
#### Comparing  probability based tools
```{r}
data.hicdcp <- fread(here(commonLoopDir, "loopScore_hicdcp.tsv")) %>%
...
data.fithic <- fread(here(commonLoopDir, "loopScore_fithic.tsv")) %>%
  dplyr::mutate(AnnoGroup = case_when(
      Anno %in% c("P-P", "P-E", "E-E") ~ "Reg (P-P, P-E, E-E)",
...
############################
data <- data.hicdcp %>% dplyr::select(AnnoGroup, ooe_DMSO, ooe_dTAG)  %>%
  dplyr::mutate(ooe_fc = ooe_dTAG/ooe_DMSO)
# ggplot(data, aes(x = ooe_DMSO, y = ooe_dTAG)) + geom_point() + facet_wrap(~AnnoGroup) + coord_fixed() + scale_x_log10() + scale_y_log10() + geom_abline(intercept = 0, slope = 1) +geom_hex(bins = 100) + scale_fill_viridis_c()


# data.long <- data %>% pivot_longer(cols = c(2, 3))
#  
# ggplot(data.long, aes(x = name, y = value)) + facet_wrap(~AnnoGroup) + geom_violin() + scale_y_log10() + geom_boxplot()


data_all <- data %>%
  mutate(AnnoGroup = "All")   # new combined panel

data2 <- bind_rows(data, data_all)
data2$AnnoGroup <- factor(data2$AnnoGroup, 
                          levels = c("All",
                                     "Reg (P-P, P-E, E-E)",
                                     "Reg/Str mixed",
                                     "Str (S-S, S-X)",
...
df_pct <- data2 %>%
  group_by(AnnoGroup) %>%
  summarize(
    pct_below1 = mean(ooe_fc < 2**(-0.5), na.rm = TRUE) * 100,
    median_fc  = median(ooe_fc, na.rm = TRUE)
  )
p <- ggplot(data2, aes(x = ooe_fc)) + geom_histogram(bins = 150) + scale_x_log10(limits = c(0.05, 7)) + facet_wrap(~AnnoGroup, nrow = 2, scales = "free")  + theme_classic() +
  geom_vline(xintercept = c(2**(-0.5), 2**(0.5)), color = "red", size = lineThick*mmToLineUnit,
               lineend = "square") +
    geom_vline(xintercept = 2**(0), color = "red", linetype = "dashed", size = lineThick*mmToLineUnit,
               lineend = "square") +

  geom_vline(
    data = df_pct,
    aes(xintercept = median_fc),
    linetype = "dotted",
    color = "white", size = lineThick*mmToLineUnit,
               lineend = "square"
  )  +
  geom_text(
    data = df_pct,
    aes(x = 0.05, y = Inf, 
        label = paste0(round(pct_below1, 1), "%\nDOWN")),
    vjust = 1.5,
    hjust = 0,
    inherit.aes = FALSE,
...
ylab("Count") + xlab("Fold change (G1_ESC_dTAG vs G1_ESC_DMSO) of observed over expected (OOE)") +
    theme(
      legend.position = "none",
      strip.text = element_text(
      size = fontSizeM,       # Or use a multiplier like 'fontSizeS * 1.2'
      family = fontType,
      color = "black"
    ),
...
fileName <- here(figDir, "histogram_ooe_fc_hicdcp")
width <- panelSize(6)*mmToInch
...
############################
data <- data.fithic %>% dplyr::select(AnnoGroup, ooe_DMSO, ooe_dTAG)  %>%
  dplyr::mutate(ooe_fc = ooe_dTAG/ooe_DMSO)
# ggplot(data, aes(x = ooe_DMSO, y = ooe_dTAG)) + geom_point() + facet_wrap(~AnnoGroup) + coord_fixed() + scale_x_log10() + scale_y_log10() + geom_abline(intercept = 0, slope = 1) +geom_hex(bins = 100) + scale_fill_viridis_c()
...
fileName <- here(figDir, "histogram_ooe_fc_fithic")
width <- panelSize(6)*mmToInch
height <- panelSize(2)*mmToInch
svglite(paste0(fileName, ".svg"),  height = height, width = width)
print(p)
...
for(expandSize in c(5000, 10000, 25000)){
  gi1 <- expand_anchor(make_gi(loop_chromo))
  gi2 <- expand_anchor(make_gi(loop_hicdcp))
  gi3 <- expand_anchor(make_gi(loop_fithic))
  ...
  p <- upset(
    membership_df,
    intersect = c("Chromosight","HiCDCP","FitHiC"),
    name = "",
    base_annotations = list(
  ...
  fileName <- here(figDir, paste0("offSet3_", expandSize, "bp_expanded"))
...
chromosight_only_idx <- membership_df %>%
  filter(Chromosight & !HiCDCP & !FitHiC) %>%
  pull(loop)
gi_chromosight_only <- gi_all[chromosight_only_idx]
fwrite(as_data_frame(gi_chromosight_only) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlap_Chromosight_unique.bedpe"), sep = "\t", col.names = FALSE)
...
gi_chromosight_only <- gi_all[chromosight_only_idx]
fwrite(as_data_frame(gi_chromosight_only) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlap_HiCDCP_unique.bedpe"), sep = "\t", col.names = FALSE)
...
gi_chromosight_only <- gi_all[chromosight_only_idx]
fwrite(as_data_frame(gi_chromosight_only) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlap_FitHiC_unique.bedpe"), sep = "\t", col.names = FALSE)
...
gi_chromosight_only <- gi_all[chromosight_only_idx]
fwrite(as_data_frame(gi_chromosight_only) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlap_HiCDCP_FitHiC.bedpe"), sep = "\t", col.names = FALSE)
...
gi_chromosight_only <- gi_all[chromosight_only_idx]
fwrite(as_data_frame(gi_chromosight_only) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlap_Chromosight_HiCDCP.bedpe"), sep = "\t", col.names = FALSE)
...
gi_chromosight_only <- gi_all[chromosight_only_idx]
fwrite(as_data_frame(gi_chromosight_only) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlap_Chromosight_FitHiC.bedpe"), sep = "\t", col.names = FALSE)
...
gi_chromosight_only <- gi_all[chromosight_only_idx]
fwrite(as_data_frame(gi_chromosight_only) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlap_Chromosight_HiCDCP_FitHiC.bedpe"), sep = "\t", col.names = FALSE)


chromosight_only_idx <- membership_df %>%
  filter((Chromosight & HiCDCP & FitHiC) |
          (Chromosight & !HiCDCP & FitHiC) |
           (Chromosight & HiCDCP & !FitHiC) ) %>%
  pull(loop)
gi_chromosight_only <- gi_all[chromosight_only_idx]
fwrite(as_data_frame(gi_chromosight_only) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
...
# 4) determine membership via Chebyshev distance = 1 bin(10 kb)
in1 <- overlapsAny(gi1, c(gi2, gi3),    maxgap=expandSize, type="equal")

gi1_raw <- (make_gi(loop_chromo))
gi_chromosight_unique <- gi1_raw[!in1]
gi_chromosight_overlap <- gi1_raw[in1]

fwrite(as_data_frame(gi_chromosight_unique) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlapChromosight_unique.bedpe"), sep = "\t", col.names = FALSE)
fwrite(as_data_frame(gi_chromosight_overlap) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlapChromosight_overlap.bedpe"), sep = "\t", col.names = FALSE)


in1 <- overlapsAny(gi2, gi1,    maxgap=expandSize, type="equal")

gi2_raw <- (make_gi(loop_hicdcp))
gi_chromosight_unique <- gi2_raw[!in1]
gi_chromosight_overlap <- gi2_raw[in1]

fwrite(as_data_frame(gi_chromosight_unique) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlapHiCDCP_unique.bedpe"), sep = "\t", col.names = FALSE)
fwrite(as_data_frame(gi_chromosight_overlap) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlapHiCDCP_overlap.bedpe"), sep = "\t", col.names = FALSE)

in1 <- overlapsAny(gi3, gi1,    maxgap=expandSize, type="equal")

gi3_raw <- (make_gi(loop_fithic))
gi_chromosight_unique <- gi3_raw[!in1]
gi_chromosight_overlap <- gi3_raw[in1]

fwrite(as_data_frame(gi_chromosight_unique) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlapFitHiC_unique.bedpe"), sep = "\t", col.names = FALSE)
fwrite(as_data_frame(gi_chromosight_overlap) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)),
       here(commonLoopDir, "loopOverlapFitHiC_overlap.bedpe"), sep = "\t", col.names = FALSE)
...
#### Subdivision based on common or not
loop_chromo_unique <- fread(here(commonLoopDir, "loopOverlapChromosight_unique.bedpe")) %>%
  dplyr::mutate(id = paste(V1, V2, V3, V4, V5, V6, sep = "_"))
loop_chromo_unique <- loop_chromo %>% dplyr::filter(id %in% loop_chromo_unique$id)
loop_chromo_common <- loop_chromo %>% dplyr::filter(!(id %in% loop_chromo_unique$id))


loop_fithic_unique <- fread(here(commonLoopDir, "loopOverlapFitHiC_unique.bedpe")) %>%
  dplyr::mutate(id = paste(V1, V2, V3, V4, V5, V6, sep = "_"))
loop_fithic_unique <- loop_fithic %>% dplyr::filter(id %in% loop_fithic_unique$id)
loop_fithic_common <- loop_fithic %>% dplyr::filter(!(id %in% loop_fithic_unique$id))


loop_hicdcp_unique <- fread(here(commonLoopDir, "loopOverlapHiCDCP_unique.bedpe")) %>%
  dplyr::mutate(id = paste(V1, V2, V3, V4, V5, V6, sep = "_"))
loop_hicdcp_unique <- loop_hicdcp %>% dplyr::filter(id %in% loop_hicdcp_unique$id)
loop_hicdcp_common <- loop_hicdcp %>% dplyr::filter(!(id %in% loop_hicdcp_unique$id))

data <- make_table(loop_chromo, "chromosight") %>%
  dplyr::full_join(make_table(loop_hicdcp, "hicdcp"), by = c("AnnoGroup")) %>%
  dplyr::full_join(make_table(loop_fithic, "fithic"), by = c("AnnoGroup")) %>%
  dplyr::full_join(make_table(loop_chromo_unique, "chromosight_unique"), by = c("AnnoGroup")) %>%
  dplyr::full_join(make_table(loop_fithic_unique, "fithic_unique"), by = c("AnnoGroup")) %>%
  dplyr::full_join(make_table(loop_hicdcp_unique, "hicdcp_unique"), by = c("AnnoGroup"))  %>%
  dplyr::full_join(make_table(loop_chromo_common, "chromosight_common"), by = c("AnnoGroup")) %>%
  dplyr::full_join(make_table(loop_fithic_common, "fithic_common"), by = c("AnnoGroup")) %>%
  dplyr::full_join(make_table(loop_hicdcp_common, "hicdcp_common"), by = c("AnnoGroup"))


data_long <- data %>% pivot_longer(cols = c("chromosight", "chromosight_common", "chromosight_unique",
                                            "fithic", "fithic_common", "fithic_unique",
                                            "hicdcp", "hicdcp_common", "hicdcp_unique"))
data_long$name <- factor(data_long$name, levels = c("chromosight", "chromosight_common", "chromosight_unique",
                                            "fithic", "fithic_common", "fithic_unique",
                                            "hicdcp", "hicdcp_common", "hicdcp_unique"))
...
fileName <- paste0("Barplot_loopAnno_comparison_uniqueCommon")
...
data <- fread(here(commonLoopDir, "loopScore_chromosight.tsv")) %>%
  dplyr::filter(id %in% loop_chromo_unique$id) %>%
  dplyr::filter(!is.na(ooe_DMSO), !is.na(ooe_dTAG)) %>%
  dplyr::mutate(
    x_ooe = if_else(ooe_DMSO == 0, pseudo_ooe, ooe_DMSO),
    y_ooe = if_else(ooe_dTAG == 0, pseudo_ooe, ooe_dTAG),
...
fileName <- paste0("dist_vs_score_binned_reg_str_chromo_unique")
...
fileName <- paste0("dist_vs_score_binned_reg_str_chromo_common")
...
data <- fread(here(commonLoopDir, "loopScore_fithic.tsv")) %>%
  dplyr::filter(!is.na(ooe_DMSO), !is.na(ooe_dTAG)) %>%
  dplyr::mutate(
    x_ooe = if_else(ooe_DMSO == 0, pseudo_ooe, ooe_DMSO),
    y_ooe = if_else(ooe_dTAG == 0, pseudo_ooe, ooe_dTAG),
...
fileName <- paste0("dist_vs_score_binned_reg_str_fithic")
...
data <- fread(here(commonLoopDir, "loopScore_fithic.tsv")) %>%
  dplyr::filter(id %in% loop_fithic_unique$id) %>%
  dplyr::filter(!is.na(ooe_DMSO), !is.na(ooe_dTAG)) %>%
  dplyr::mutate(
    x_ooe = if_else(ooe_DMSO == 0, pseudo_ooe, ooe_DMSO),
    y_ooe = if_else(ooe_dTAG == 0, pseudo_ooe, ooe_dTAG),
...
fileName <- paste0("dist_vs_score_binned_reg_str_fithic_unique")
...
fileName <- paste0("dist_vs_score_binned_reg_str_fithic_common")
...
fileName <- paste0("dist_vs_score_binned_reg_str_hicdcp")
...
data <- fread(here(commonLoopDir, "loopScore_hicdcp.tsv")) %>%
  dplyr::filter(id %in% loop_hicdcp_unique$id) %>%
  dplyr::filter(!is.na(ooe_DMSO), !is.na(ooe_dTAG)) %>%
  dplyr::mutate(
    x_ooe = if_else(ooe_DMSO == 0, pseudo_ooe, ooe_DMSO),
    y_ooe = if_else(ooe_dTAG == 0, pseudo_ooe, ooe_dTAG),
...
fileName <- paste0("dist_vs_score_binned_reg_str_hicdcp_unique")
...
fileName <- paste0("dist_vs_score_binned_reg_str_hicdcp_common")
...
pseudo_ooe    <- 0.01
palnpg        <- pal_npg()(10)
# Define specific colors mapped to groups to ensure consistency
group_colors  <- c("P-P" = palnpg[8], "P-E" = palnpg[1], "E-E" = palnpg[5], 
                   "mixed" = palnpg[2], "str" = palnpg[3])

# --- Function 1: Load and Process Data ---
# mode: "all" (no filtering), "include" (keep only ids in id_list), "exclude" (remove ids in id_list)
get_processed_data <- function(file_path, id_list = NULL, mode = "all") {
  
  dt <- fread(file_path) %>%
    dplyr::filter(!is.na(ooe_DMSO), !is.na(ooe_dTAG))
  
  # Filter based on ID list
  if (mode == "include" && !is.null(id_list)) {
    dt <- dt %>% dplyr::filter(id %in% id_list)
  } else if (mode == "exclude" && !is.null(id_list)) {
    dt <- dt %>% dplyr::filter(!id %in% id_list)
  }
  ...
  dt <- dt %>%
    ...
    dplyr::mutate(
      group2 = case_when(
        Anno %in% c("P-P", "P-E", "E-E") ~ Anno,
        group %in% c("reg_strReg", "reg_str", "strReg_strReg", "strReg_str", "strReg_other") ~ "mixed",
        group == "str" ~ "str",
        TRUE ~ "XXXXX"
      )
    )
  
  # Factorize and select final columns
  dt$group2 <- factor(dt$group2, levels = c("P-P", "P-E", "E-E", "mixed", "str"))
  
  return(dt)
}

# --- Function 2: Plot and Save ---
plot_loop_smooth <- function(data, y_lims, filename_suffix) {
  
  # Filter for plotting (distance < 550kb and valid groups)
  plot_data <- data %>% 
    dplyr::filter(group2 %in% names(group_colors), distance < 550000)
  
  p <- ggplot(plot_data, aes(x = distance, y = fc, fill = group2, color = group2)) +
    geom_smooth(show.legend = TRUE, se = TRUE, 
                size = lineThick * mmToLineUnit, lineend = "square") +
    scale_fill_manual(values = group_colors) +
    scale_color_manual(values = group_colors) +
    geom_hline(yintercept = 0) +
    coord_cartesian(ylim = y_lims) +
    scale_x_continuous(labels = label_kb_mb) +
    labs(x = "Size (bp)", y = "Δ log2(obs/exp)") +
    theme_classic() +
    theme(
      axis.text.x      = element_text(angle = 45, hjust = 1),
      strip.background = element_rect(fill = "white", color = "black"),
  ...
  width  <- panelSize(2.2) * mmToInch
  height <- panelSize(1.5) * mmToInch
  full_path <- here(figDir, paste0("dist_vs_score_binned_reg_str_", filename_suffix, "_500kb.svg"))
  
  svglite(full_path, width = width, height = height)
  print(p)
  dev.off()
}
```
...
```{r}
file_chromo <- here(commonLoopDir, "loopScore_chromosight.tsv")
ymin = -1.8
ymax = -0.2

df_chromo_all <- get_processed_data(file_chromo, mode = "all")
plot_loop_smooth(df_chromo_all, y_lims = c(ymin, ymax), filename_suffix = "chromo")

# 1. Unique Loops
df_chromo_unique <- get_processed_data(file_chromo, id_list = loop_chromo_unique$id, mode = "include")
plot_loop_smooth(df_chromo_unique, y_lims = c(ymin, ymax), filename_suffix = "chromo_unique")

# 2. Common Loops (Not Unique)
df_chromo_common <- get_processed_data(file_chromo, id_list = loop_chromo_unique$id, mode = "exclude")
plot_loop_smooth(df_chromo_common, y_lims = c(ymin, ymax), filename_suffix = "chromo_common")

file_fithic <- here(commonLoopDir, "loopScore_fithic.tsv")

# 1. All Loops (Note: Original code had tight ylims for "All" plot)
df_fithic_all <- get_processed_data(file_fithic, mode = "all")
plot_loop_smooth(df_fithic_all, y_lims = c(ymin, ymax), filename_suffix = "fithic")

# 2. Unique Loops (Note: Original code had wider ylims for "Unique" plot)
df_fithic_unique <- get_processed_data(file_fithic, id_list = loop_fithic_unique$id, mode = "include")
plot_loop_smooth(df_fithic_unique, y_lims = c(ymin, ymax), filename_suffix = "fithic_unique")

# 3. Common Loops
df_fithic_common <- get_processed_data(file_fithic, id_list = loop_fithic_unique$id, mode = "exclude")
plot_loop_smooth(df_fithic_common, y_lims = c(ymin, ymax), filename_suffix = "fithic_common")

file_hicdcp <- here(commonLoopDir, "loopScore_hicdcp.tsv")

# 1. All Loops
df_hicdcp_all <- get_processed_data(file_hicdcp, mode = "all")
plot_loop_smooth(df_hicdcp_all, y_lims = c(ymin, ymax), filename_suffix = "hicdcp")

# 2. Unique Loops
df_hicdcp_unique <- get_processed_data(file_hicdcp, id_list = loop_hicdcp_unique$id, mode = "include")
plot_loop_smooth(df_hicdcp_unique, y_lims = c(ymin, ymax), filename_suffix = "hicdcp_unique")

# 3. Common Loops
df_hicdcp_common <- get_processed_data(file_hicdcp, id_list = loop_hicdcp_unique$id, mode = "exclude")
plot_loop_smooth(df_hicdcp_common, y_lims = c(ymin, ymax), filename_suffix = "hicdcp_common")
```
...
### [Rev2] Enrichment of EpiLC specific H3K27ac on loops
```{r}
peak.H3K27ac.EpiLCspecific <- importPeak(here(refDir, "ChIP", "differential", "H3K27ac_diff_specific_EpiLC.bed"))
...
length(subjectHits(findOverlaps(anchor.all, peak.H3K27ac.EpiLCspecific)))
length(subjectHits(findOverlaps(anchor.upno, peak.H3K27ac.EpiLCspecific)))
length(subjectHits(findOverlaps(anchor.down, peak.H3K27ac.EpiLCspecific)))
...
dev.off()
```
### [Rev2] Grouping based on presence of DOWN loop
```{r}
alpha <- 0.05
fcCutoff <- 0.5
diffCutoff <- 0.2
...
ks_result1 <- ks.test(
  temp %>% dplyr::filter(flag_low_diff) %>% pull(absLog2FC),
  temp %>% dplyr::filter(!flag_low_diff) %>% pull(absLog2FC)
)
convPvalue(ks_result1$p.value)

p <- ggplot(temp, aes(x = absLog2FC, color = flag_low_diff)) +
  scale_color_manual(values = rev(c("#F28E2C", "#777777"))) +
...
fileName <- paste0("DOWNloopPresence_avgLog2FC_cdf")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p)
...
```
##### Plot TAD CDF per replicate
```{r}
diff.RNA.all <- fread(here(diffDir, "diff_G1.dTAG_G1.2i.dTAG_vs_G1.2i.DMSO.tsv")) %>%
  dplyr::mutate(baseline = rowMeans(.[, 11:13], na.rm = TRUE)) %>%
  dplyr::mutate(naiveLog2FC_rep1 =
                  log2(((S04_R030_G1.2i.dTAG_rep1_batch2 +
                           S05_R030_G1.2i.dTAG_rep1_batch3)/2 +1)/(baseline+1)),
                naiveLog2FC_rep2 = log2(((S06_R030_G1.2i.dTAG_rep2_batch2 +
                                            S07_R030_G1.2i.dTAG_rep2_batch3)/2 +1)/(baseline+1))) %>%
  dplyr::select(ensembl_gene_id, naiveLog2FC_rep1, naiveLog2FC_rep2)


temp <- left_join(dataToPlot, diff.RNA.all, by = c("ensembl" = "ensembl_gene_id")) %>% dplyr::filter(!is.na(naiveLog2FC_rep1), !is.na(naiveLog2FC_rep2), !is.na(tadID))


dataToPlot2 <- temp %>% group_by(tadID) %>%
  dplyr::summarise(absAvgLog2FC1 = abs(mean(naiveLog2FC_rep1)),
                   absAvgLog2FC2 = abs(mean(naiveLog2FC_rep2)),
                   tadGeneDensity = mean(tadGeneDensity))
...
group1_data <- dataToPlot2$absAvgLog2FC1[dataToPlot2$geneDensityGroup == "0-20%"]
group2_data <- dataToPlot2$absAvgLog2FC1[dataToPlot2$geneDensityGroup == "20-40%"]
ks_result1 <- ks.test(group1_data, group2_data)

group1_data <- dataToPlot2$absAvgLog2FC1[dataToPlot2$geneDensityGroup == "0-20%"]
group2_data <- dataToPlot2$absAvgLog2FC1[dataToPlot2$geneDensityGroup == "40-60%"]
ks_result2 <- ks.test(group1_data, group2_data)

group1_data <- dataToPlot2$absAvgLog2FC1[dataToPlot2$geneDensityGroup == "0-20%"]
group2_data <- dataToPlot2$absAvgLog2FC1[dataToPlot2$geneDensityGroup == "60-80%"]
ks_result3 <- ks.test(group1_data, group2_data)

group1_data <- dataToPlot2$absAvgLog2FC1[dataToPlot2$geneDensityGroup == "0-20%"]
group2_data <- dataToPlot2$absAvgLog2FC1[dataToPlot2$geneDensityGroup == "80-100%"]
ks_result4 <- ks.test(group1_data, group2_data)

print(convPvalue(ks_result1$p.value))
print(convPvalue(ks_result2$p.value))
print(convPvalue(ks_result3$p.value))
print(convPvalue(ks_result4$p.value))

p1 <- ggplot(dataToPlot2, aes(x = absAvgLog2FC1, color = geneDensityGroup)) +
  scale_color_manual(values = rev(c("#777777", "#8B7E65", "#A28452", "#C2884D", "#F28E2C"))) +
  stat_ecdf(size = 0.4, linewidth = lineMedium * mmToLineUnit, lineend = "square" ) + # Use stat_ecdf to plot the empirical CDF
  labs(
    x = "Abs. log2(fold change)",
...
fileName <- paste0("TAD_tadGroup_geneDensity_avgLog2FC_cdf_rep1")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p1)
...
group1_data <- dataToPlot2$absAvgLog2FC2[dataToPlot2$geneDensityGroup == "0-20%"]
group2_data <- dataToPlot2$absAvgLog2FC2[dataToPlot2$geneDensityGroup == "20-40%"]
ks_result1 <- ks.test(group1_data, group2_data)

group1_data <- dataToPlot2$absAvgLog2FC2[dataToPlot2$geneDensityGroup == "0-20%"]
group2_data <- dataToPlot2$absAvgLog2FC2[dataToPlot2$geneDensityGroup == "40-60%"]
ks_result2 <- ks.test(group1_data, group2_data)

group1_data <- dataToPlot2$absAvgLog2FC2[dataToPlot2$geneDensityGroup == "0-20%"]
group2_data <- dataToPlot2$absAvgLog2FC2[dataToPlot2$geneDensityGroup == "60-80%"]
ks_result3 <- ks.test(group1_data, group2_data)

group1_data <- dataToPlot2$absAvgLog2FC2[dataToPlot2$geneDensityGroup == "0-20%"]
group2_data <- dataToPlot2$absAvgLog2FC2[dataToPlot2$geneDensityGroup == "80-100%"]
ks_result4 <- ks.test(group1_data, group2_data)
...
p2 <- ggplot(dataToPlot2, aes(x = absAvgLog2FC2, color = geneDensityGroup)) +
  scale_color_manual(values = rev(c("#777777", "#8B7E65", "#A28452", "#C2884D", "#F28E2C"))) +
  stat_ecdf(size = 0.4, linewidth = lineMedium * mmToLineUnit, lineend = "square" ) + # Use stat_ecdf to plot the empirical CDF
  labs(
    x = "Abs. log2(fold change)",
...
fileName <- paste0("TAD_tadGroup_geneDensity_avgLog2FC_cdf_rep2")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p2)
...
temp2 <- readRDS(here("../data", "rds", "geneLoopNum.rds"))
...
temp <- left_join(temp, diff.RNA.all, by = c("gene" = "ensembl_gene_id"))
temp <- temp %>% dplyr::mutate(absNaiveLog2FC_rep1 = abs(naiveLog2FC_rep1),
                               absNaiveLog2FC_rep2 = abs(naiveLog2FC_rep2))

ks_result1 <- ks.test(
  temp %>% dplyr::filter(psNum == "psOver4") %>% pull(absNaiveLog2FC_rep1),
  temp %>% dplyr::filter(psNum == "ps3") %>% pull(absNaiveLog2FC_rep1)
)
ks_result2 <- ks.test(
  temp %>% dplyr::filter(psNum == "psOver4") %>% pull(absNaiveLog2FC_rep1),
  temp %>% dplyr::filter(psNum == "ps2") %>% pull(absNaiveLog2FC_rep1)
)
ks_result3 <- ks.test(
  temp %>% dplyr::filter(psNum == "psOver4") %>% pull(absNaiveLog2FC_rep1),
  temp %>% dplyr::filter(psNum == "ps1") %>% pull(absNaiveLog2FC_rep1)
)
ks_result4 <- ks.test(
  temp %>% dplyr::filter(psNum == "psOver4") %>% pull(absNaiveLog2FC_rep1),
  temp %>% dplyr::filter(psNum == "ps0") %>% pull(absNaiveLog2FC_rep1)
)
print(convPvalue(ks_result1$p.value))
print(convPvalue(ks_result2$p.value))
print(convPvalue(ks_result3$p.value))
print(convPvalue(ks_result4$p.value))
p1 <- ggplot(temp, aes(x = absNaiveLog2FC_rep1, color = psNum)) +
  scale_color_manual(values = (c("#777777", "#8B7E65", "#A28452", "#C2884D", "#F28E2C"))) +
  stat_ecdf(size = 0.4, linewidth = lineMedium * mmToLineUnit, lineend = "square" ) + # Use stat_ecdf to plot the empirical CDF
  labs(
    x = "Abs. log2(fold change)",
...
ks_result1 <- ks.test(
  temp %>% dplyr::filter(psNum == "psOver4") %>% pull(absNaiveLog2FC_rep2),
  temp %>% dplyr::filter(psNum == "ps3") %>% pull(absNaiveLog2FC_rep2)
)
ks_result2 <- ks.test(
  temp %>% dplyr::filter(psNum == "psOver4") %>% pull(absNaiveLog2FC_rep2),
  temp %>% dplyr::filter(psNum == "ps2") %>% pull(absNaiveLog2FC_rep2)
)
ks_result3 <- ks.test(
  temp %>% dplyr::filter(psNum == "psOver4") %>% pull(absNaiveLog2FC_rep2),
  temp %>% dplyr::filter(psNum == "ps1") %>% pull(absNaiveLog2FC_rep2)
)
ks_result4 <- ks.test(
  temp %>% dplyr::filter(psNum == "psOver4") %>% pull(absNaiveLog2FC_rep2),
  temp %>% dplyr::filter(psNum == "ps0") %>% pull(absNaiveLog2FC_rep2)
)
print(convPvalue(ks_result1$p.value))
print(convPvalue(ks_result2$p.value))
print(convPvalue(ks_result3$p.value))
print(convPvalue(ks_result4$p.value))
p2 <- ggplot(temp, aes(x = absNaiveLog2FC_rep2, color = psNum)) +
  scale_color_manual(values = (c("#777777", "#8B7E65", "#A28452", "#C2884D", "#F28E2C"))) +
  stat_ecdf(size = 0.4, linewidth = lineMedium * mmToLineUnit, lineend = "square" ) + # Use stat_ecdf to plot the empirical CDF
  labs(
    x = "Abs. log2(fold change)",
...
fileName <- paste0("psGroup_abslog2FC_dTAGvsDMSO_rep1")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p1)
...
fileName <- paste0("psGroup_abslog2FC_dTAGvsDMSO_rep2")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p2)
...
print(convPvalue(ks_result$p.value))
...
diff.RNA.all <- fread(here(diffDir, "diff_G1.dTAG_G1.2i.dTAG_vs_G1.2i.DMSO.tsv")) %>%
  ...
  dplyr::mutate(naiveLog2FC_rep1 =
                ...
                naiveLog2FC_rep2 = log2(((S06_R030_G1.2i.dTAG_rep2_batch2 +
                                            S07_R030_G1.2i.dTAG_rep2_batch3)/2 +1)/(baseline+1))) %>%
  dplyr::select(ensembl_gene_id, naiveLog2FC_rep1, naiveLog2FC_rep2) %>%
  plyr::mutate(absNaiveLog2FC_rep1 = abs(naiveLog2FC_rep1),
                               absNaiveLog2FC_rep2 = abs(naiveLog2FC_rep2)) %>%
  dplyr::mutate(group = case_when(
    ensembl_gene_id %in% gene.wBoundary ~ "withinBoundary",
    ensembl_gene_id %in% gene.noBoundary ~ "noBoundary",
...
ks_result <- ks.test(
  diff.RNA.all %>% dplyr::filter(group == "withinBoundary") %>% pull(absNaiveLog2FC_rep1),
  diff.RNA.all %>% dplyr::filter(group == "noBoundary") %>% pull(absNaiveLog2FC_rep1)
)
print(convPvalue(ks_result$p.value))

# Create the CDF plot
p1 <- ggplot(diff.RNA.all, aes(x = absNaiveLog2FC_rep1, color = group)) +
  scale_color_manual(values = (c("#777777", "#F28E2C"))) +
...
fileName <- paste0("log2FC_cdf_insulationBoundary_convSS_pe_rep1")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p1)
...
ks_result <- ks.test(
  diff.RNA.all %>% dplyr::filter(group == "withinBoundary") %>% pull(absNaiveLog2FC_rep2),
  diff.RNA.all %>% dplyr::filter(group == "noBoundary") %>% pull(absNaiveLog2FC_rep2)
)
print(convPvalue(ks_result$p.value))

# Create the CDF plot
p2 <- ggplot(diff.RNA.all, aes(x = absNaiveLog2FC_rep2, color = group)) +
  scale_color_manual(values = (c("#777777", "#F28E2C"))) +
...
fileName <- paste0("log2FC_cdf_insulationBoundary_convSS_pe_rep2")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p2)
...
```
### Chromosight vs HiCDC+ vs FitHiC
```{r}
...
res = 5000
# Resolution 5kb
width <- panelSize(2.2)*mmToInch * scalingFactor
height <- panelSize(2.2)*mmToInch * scalingFactor

temp <- loop_chromo
colnames(temp) <- c("V1", "V2", "V3", "V4", "V5", "V6")
loops <- importBedpe(temp)
hic1 <- import(cf.G1DMSO, focus = paste0(chr, ":", start, "-", end), resolution = res)
p1 <- plotMatrix(hic1, dpi = 600, loop = loops, limits = c(0, zmax)) +
  theme(
    legend.position = "none",
    plot.title = element_text(
      hjust = 0.5,
      size = fontSizeS* scalingFactor,
      family = fontType
    ),
    axis.title = element_text(
      size = fontSizeS* scalingFactor,
...
fileName <- here(figVisdir, paste0("loopCallerCompare_chromosight_", i, "_zmax", zmax))
svglite(paste0(fileName, ".svg"), width = width, height = height)
print(p1)
dev.off()
...
loops <- importBedpe(temp)
hic1 <- import(cf.G1DMSO, focus = paste0(chr, ":", start, "-", end), resolution = res)
p2 <- plotMatrix(hic1, dpi = 600, loop = loops, limits = c(0, zmax)) +
  theme(
    legend.position = "none",
    plot.title = element_text(
      hjust = 0.5,
...
fileName <- here(figVisdir, paste0("loopCallerCompare_fithic_", i, "_zmax", zmax))
svglite(paste0(fileName, ".svg"), width = width, height = height)
print(p2)
dev.off()
...
loops <- importBedpe(temp)
hic1 <- import(cf.G1DMSO, focus = paste0(chr, ":", start, "-", end), resolution = res)
p3 <- plotMatrix(hic1, dpi = 600, loop = loops, limits = c(0, zmax)) +
  theme(
    legend.position = "none",
    plot.title = element_text(
      hjust = 0.5,
...
fileName <- here(figVisdir, paste0("loopCallerCompare_hicdcp_",i, "_zmax", zmax ))
svglite(paste0(fileName, ".svg"), width = width, height = height)
print(p3)
dev.off()
...
## Plotting strength of the conserved loops
temp <- plot_df_delta %>% dplyr::filter(delta_Async < -0.2, 
                                        abs(delta_G1) < 0.2,
                                        delta_Async - delta_G1 < -0.2)
```
...
g1specific <- plot_df_delta %>% dplyr::filter(delta_G1 < -0.2, 
                                        abs(delta_Async) < 0.2,
                                        delta_G1 - delta_Async < -0.2)
asyncspecific <- plot_df_delta %>% dplyr::filter(delta_Async < -0.2, 
                                        abs(delta_G1) < 0.2,
                                        delta_Async - delta_G1 < -0.2)
print(nrow(g1specific))
...
plot_df_delta <- plot_df_delta %>% dplyr::mutate(
  group = case_when(
    id %in% ncBoth$id ~ "Common Resilient",
    id %in% g1specific$id ~ "G1 specific",
    id %in% asyncspecific$id ~ "Async specific",

    TRUE ~ NA
  )
)

temp <- plot_df_delta %>% dplyr::filter(!is.na(group))

diffCutoff = 0.2

# p0 <- ggplot(temp, aes(x = score_DMSO, y = score_UT)) + geom_point() + coord_fixed(xlim = c(-0.5, 1), ylim = c(-0.5, 1)) + facet_wrap(~group) + geom_abline(intercept = 0, abline = 1)

p <- ggplot(temp, aes(x = score_DMSO, y = score_UT)) +
  ...
  theme_classic() + 
  labs(x = "Score (G1 ESC DMSO)", y = "Score (Async ESC UT)") + 
  geom_abline(slope = 1, intercept = 0, col = "grey50", linetype = "dashed",
              color = "black",
              size = lineThick*mmToLineUnit,
              lineend = "square") +
...
name <- "scatterplot_G1vsAsync_AsyncCalled"
width <- panelSize(3)*mmToInch
height <- panelSize(2)*mmToInch  
svglite(here(figDir,
             paste0(name, ".svg")),
...
temp_long <- temp %>% dplyr::select(group, score_DMSO, score_UT) %>% pivot_longer(cols = c(2, 3))
p1 <- ggplot(temp_long, aes(x = name, y = value)) + geom_violin() + facet_wrap(~group) + geom_boxplot()
pseudo_ooe <- 0.01
plot_df_ooe <- temp %>%
  dplyr::select(id, ooe_DMSO, ooe_UT, group) %>%
  dplyr::filter(!is.na(ooe_DMSO), !is.na(ooe_UT)) %>%
  dplyr::mutate(
    ooe_DMSO = if_else(ooe_DMSO == 0, pseudo_ooe, ooe_DMSO),
    ooe_UT = if_else(ooe_UT == 0, pseudo_ooe, ooe_UT),
    fc    = log2(ooe_UT / ooe_DMSO)
  )

plot_df_ooe$group <- factor(plot_df_ooe$group, levels = c("Async specific", "Common Resilient", "G1 specific"))

getPvalWilcox <- function(data, group1, group2){
  distance1 <- (data %>% dplyr::filter(group == group1) )$fc
  distance2 <- (data %>% dplyr::filter(group == group2) )$fc
  wil <- wilcox.test(distance1, distance2)
  return(wil)
}
getPvalWilcox(plot_df_ooe, "G1 specific", "Common Resilient")
getPvalWilcox(plot_df_ooe, "G1 specific", "Async specific")
getPvalWilcox(plot_df_ooe, "Async specific", "Common Resilient")


p3 <- ggplot(plot_df_ooe, aes(x = group, y = fc, fill = group)) +
  geom_violin(linewidth = lineThick * mmToLineUnit, lineend = "square", alpha = .4, show.legend = FALSE,
              color = "black") + 
  geom_boxplot(width = 0.3, color = "black",
               linewidth = lineThick * mmToLineUnit, lineend = "square",
...
width <- panelSize(1.1)*mmToInch
height <- panelSize(1.5)*mmToInch
fileName <- paste0("violin_", "G1vsAsync_AsyncCalled")
svglite(here(figDir, paste0(fileName, ".svg")),  height = height, width = width)
print(p3)
dev.off()  
...
# p2 <- ggplot(temp, aes(x = score_DMSO, y = score_UT)) + geom_point() + coord_fixed(xlim = c(-0.5, 1), ylim = c(-0.5, 1)) + facet_wrap(~group) + geom_abline(intercept = 0, abline = 1)
...
name <- "scatterplot_G1vsAsync_G1Called"
width <- panelSize(3)*mmToInch
height <- panelSize(2)*mmToInch  
svglite(here(figDir,
             paste0(name, ".svg")),
...
# temp_long <- temp %>% dplyr::select(group, score_DMSO, score_UT) %>% pivot_longer(cols = c(2, 3))
# p3<- ggplot(temp_long, aes(x = name, y = value)) + geom_violin() + facet_wrap(~group) + geom_boxplot()
# 
...
width <- panelSize(1.1)*mmToInch
height <- panelSize(1.5)*mmToInch
fileName <- paste0("violin_", "G1vsAsync_G1Called")
svglite(here(figDir, paste0(fileName, ".svg")),  height = height, width = width)
print(p3)
dev.off()  
...
data <- fread(here(commonLoopDir, "loopScore_chromosight.tsv"))
data <- data %>% dplyr::mutate(delta = score_dTAG - score_DMSO) %>%
    dplyr::mutate(AnnoGroup = case_when(
      Anno %in% c("P-P", "P-E", "E-E") ~ "Reg (P-P, P-E, E-E)",
...
pseudo_ooe <- 0.01
plot_df_ooe <- data %>%
  dplyr::select(id, ooe_DMSO, ooe_dTAG, AnnoGroup) %>%
  dplyr::filter(!is.na(ooe_DMSO), !is.na(ooe_dTAG)) %>%
  dplyr::mutate(
    ooe_DMSO = if_else(ooe_DMSO == 0, pseudo_ooe, ooe_DMSO),
    ooe_dTAG = if_else(ooe_dTAG == 0, pseudo_ooe, ooe_dTAG),
    fc    = log2(ooe_dTAG / ooe_DMSO)
  )

data$AnnoGroup <- factor(data$AnnoGroup, levels = c("Reg (P-P, P-E, E-E)", "Reg/Str mixed", "Str (S-S, S-X)",
                                                    "Reg-Other", "StrReg-Other", "Other"))

p2 <- ggplot(data, aes(x = score_DMSO, y = delta)) +
  coord_fixed(xlim = c(-0.5, 1), ylim = c(-1, 1)) + facet_wrap(~AnnoGroup, nrow = 1) +
  # geom_point(alpha=0.1, size = 1) +
  geom_hex(bins = 100) +
  scale_fill_viridis_c() +
  # coord_fixed(xlim = c(-0.5, 1), ylim = c(-0.5, 1)) +
  theme_classic() + 
  labs(x = "Score (G1 ESC DMSO)", y = "delta") + 
  geom_hline(yintercept = 0, alpha = 0.5, color = "grey",
             color = "black",
             size = lineThick*mmToLineUnit,
             lineend = "square") +
...
width <- panelSize(5)*mmToInch
height <- panelSize(2)*mmToInch
fileName <- paste0("Scatterplot_", "Strength_vs_loss_score")
svglite(here(figDir, paste0(fileName, ".svg")),  height = height, width = width)
print(p2)
dev.off()  

plot_df_ooe$AnnoGroup <- factor(plot_df_ooe$plot_df_ooe, levels = c("Reg (P-P, P-E, E-E)", "Reg/Str mixed", "Str (S-S, S-X)",
                                                    "Reg-Other", "StrReg-Other", "Other"))


p3 <- ggplot(plot_df_ooe, aes(x = ooe_DMSO, y = fc)) + facet_wrap(~AnnoGroup, nrow = 1) + scale_x_log10(limits = c(0.1, 20)) +
    geom_hex(bins = 100) +
  scale_fill_viridis_c() +
  scale_y_continuous(limits = c(-5, 5)) +
  theme_classic() + 
  labs(x = "OOE (G1 ESC DMSO)", y = "log2fc of obs/exp (dTAG vs DMSO)") + 
  geom_hline(yintercept = 0, alpha = 0.5, color = "grey",
             color = "black",
             size = lineThick*mmToLineUnit,
             lineend = "square") +
...
width <- panelSize(5)*mmToInch
height <- panelSize(1.5)*mmToInch
fileName <- paste0("Scatterplot_", "Strength_vs_loss_OOE")
svglite(here(figDir, paste0(fileName, ".svg")),  height = height, width = width)
print(p3)
dev.off()  
...
g1specific <- geneAnnoData %>% dplyr::filter(delta_G1 < -0.2, 
                                        abs(delta_Async) < 0.2,
                                        delta_G1 - delta_Async < -0.2)
asyncspecific <- geneAnnoData %>% dplyr::filter(delta_Async < -0.2, 
                                        abs(delta_G1) < 0.2,
                                        delta_Async - delta_G1 < -0.2)

ncBoth <- geneAnnoData %>% dplyr::filter(abs(delta_Async) < 0.2, 
                                        abs(delta_G1) < 0.2)
...
geneAnnoData <- geneAnnoData %>% dplyr::mutate(
  ...
  dplyr::filter(Anno %in% c("P-P", "P-E", "E-E"),
                !is.na(group))
result <- geneAnnoData %>% dplyr::select(group, gene) %>%
  unnest(gene) %>% 
  dplyr::count(gene, group) %>%
  pivot_wider(names_from = group, values_from = n, values_fill = 0)
final_result <- result %>%
  mutate(group = case_when(
    `G1 specific` >= 1 & `Async specific` == 0 ~ "G1",
    `Async specific` >= 1 & `G1 specific` == 0 ~ "Async",
    `Common Resilient` >= 1 ~ "common",
    TRUE ~ "other" 
  ))
...
temp <- diff.RNA %>% left_join(final_result %>% dplyr::select(gene, group), by = c("ensembl_gene_id" = "gene")) %>%
  drop_na(group)

# 
ks_result1 <- ks.test(
  temp %>% dplyr::filter(group == "common") %>% pull(absLog2FC),
  temp %>% dplyr::filter(group == "G1") %>% pull(absLog2FC)
)
ks_result2 <- ks.test(
  temp %>% dplyr::filter(group == "common") %>% pull(absLog2FC),
  temp %>% dplyr::filter(group == "Async") %>% pull(absLog2FC)
)
ks_result3 <- ks.test(
  temp %>% dplyr::filter(group == "G1") %>% pull(absLog2FC),
  temp %>% dplyr::filter(group == "Async") %>% pull(absLog2FC)
...
temp$group <- factor(temp$group, levels = c("Async", "common", "G1"))
p <- ggplot(temp, aes(x = absLog2FC, color = group)) +
  scale_color_manual(values = (c("#00a087", "#4dbbd5", "#e64b35"))) +
  stat_ecdf(size = 0.4, linewidth = lineMedium * mmToLineUnit, lineend = "square" ) + # Use stat_ecdf to plot the empirical CDF
  labs(
    x = "Abs. log2(fold change)",
    y = "Cumulative Probability"
...
# 
# 
fileName <- paste0("G1specific_abslog2FC_dTAGvsDMSO")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p)
...
### Plotting CDF
diff.RNA <- fread(here(dataDir, "../RNA_pub", "Hsieh_2022_RAD21_3h.csv")) %>% 
  dplyr::mutate(ensembl_gene_id = str_replace(ens_gene, "\\.[0-9]*$", "")) %>%
  dplyr::select(ensembl_gene_id, log2FoldChange, padj, ext_gene) %>%
   dplyr::mutate(absLog2FC = abs(log2FoldChange))
...
# 
# 
fileName <- paste0("G1specific_abslog2FC_dTAGvsDMSO_onAsync")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p)
...
### Plotting CDF
diff.RNA <- fread(here(diffDir,"../RNA_pub", "Narita_2025_RAD21.csv")) %>%
  dplyr::mutate(transcriptId = str_replace(transcriptId, "\\.[0-9]*$", ""))

mm10.ensembl2gene = as_tibble(
  getBM(
    mart = ensembl.v102,
    values = diff.RNA$transcriptId,
...
diff.RNA <- diff.RNA %>%
  dplyr::full_join(mm10.ensembl2gene, by = c("transcriptId" = "ensembl_transcript_id")) %>%
  dplyr::mutate(absLog2FC = abs(EUSeq.AID.Rad21.ESC_IAA_500uM_TC240_logFC))
  
fwrite(diff.RNA, here(diffDir,"../RNA_pub", "Narita_2025_RAD21_processed.csv"))
...
# 
# 
fileName <- paste0("G1specific_abslog2FC_dTAGvsDMSO_onAsync_Chaudhury")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p)
...
temp$group <- factor(temp$group, levels = c("looped", "proxLooped", "prox", "noSE"))
p <- ggplot(temp, aes(x = absLog2FC, color = group)) +
  scale_color_manual(values = (c("#3c5488", "#00a087", "#4dbbd5", "#e64b35"))) +
  stat_ecdf(size = 0.4, linewidth = lineMedium * mmToLineUnit, lineend = "square" ) + # Use stat_ecdf to plot the empirical CDF
  labs(
    x = "Abs. log2(fold change)",
    y = "Cumulative Probability"
...
temp <- left_join(temp, diff.RNA.all, by = c("ensembl_gene_id"))
temp <- temp %>% dplyr::mutate(absNaiveLog2FC_rep1 = abs(naiveLog2FC_rep1),
                               absNaiveLog2FC_rep2 = abs(naiveLog2FC_rep2))
...
ks_result1 <- ks.test(
  temp %>% dplyr::filter(group == "noSE") %>% pull(absNaiveLog2FC_rep1),
  temp %>% dplyr::filter(group == "prox") %>% pull(absNaiveLog2FC_rep1)
)
ks_result2 <- ks.test(
  temp %>% dplyr::filter(group == "noSE") %>% pull(absNaiveLog2FC_rep1),
  temp %>% dplyr::filter(group == "proxLooped") %>% pull(absNaiveLog2FC_rep1)
)
ks_result3 <- ks.test(
  temp %>% dplyr::filter(group == "noSE") %>% pull(absNaiveLog2FC_rep1),
  temp %>% dplyr::filter(group == "looped") %>% pull(absNaiveLog2FC_rep1)
)
...
p1 <- ggplot(temp, aes(x = absNaiveLog2FC_rep1, color = group)) +
  scale_color_manual(values = (c("#3c5488", "#00a087", "#4dbbd5", "#e64b35"))) +
  stat_ecdf(size = 0.4, linewidth = lineMedium * mmToLineUnit, lineend = "square" ) + # Use stat_ecdf to plot the empirical CDF
  labs(
    x = "Abs. log2(fold change)",
...
# 
# 
fileName <- paste0("SEoverlap_abslog2FC_dTAGvsDMSO_rep1")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p1)
...
ks_result1 <- ks.test(
  temp %>% dplyr::filter(group == "noSE") %>% pull(absNaiveLog2FC_rep2),
  temp %>% dplyr::filter(group == "prox") %>% pull(absNaiveLog2FC_rep2)
)
ks_result2 <- ks.test(
  temp %>% dplyr::filter(group == "noSE") %>% pull(absNaiveLog2FC_rep2),
  temp %>% dplyr::filter(group == "proxLooped") %>% pull(absNaiveLog2FC_rep2)
)
ks_result3 <- ks.test(
  temp %>% dplyr::filter(group == "noSE") %>% pull(absNaiveLog2FC_rep2),
  temp %>% dplyr::filter(group == "looped") %>% pull(absNaiveLog2FC_rep2)
)
...
p2 <- ggplot(temp, aes(x = absNaiveLog2FC_rep2, color = group)) +
  scale_color_manual(values = (c("#3c5488", "#00a087", "#4dbbd5", "#e64b35"))) +
  stat_ecdf(size = 0.4, linewidth = lineMedium * mmToLineUnit, lineend = "square" ) + # Use stat_ecdf to plot the empirical CDF
  labs(
    x = "Abs. log2(fold change)",
...
# 
# 
fileName <- paste0("SEoverlap_abslog2FC_dTAGvsDMSO_rep2")
width <- 33*mmToInch
height <-33*mmToInch
png(here(figDir, paste0(fileName, ".png")), res = 600, unit = "in", height = height, width = width)
print(p2)
